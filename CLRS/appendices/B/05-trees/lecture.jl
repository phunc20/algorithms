...
# ╔═╡ 7c78684c-5608-4184-b7ca-9fe42c09f3f9
begin
  using PlutoUI
  #using LaTeXStrings
  using TikzPictures
...
Now, assume that the statement holds for ``\abs{V} = 1, 2, 3, \ldots, k``. Let ``G = (V, E)`` be an acyclic undirected graph with ``\abs{V} = k+1``.
First, we would like to
01. Find some vertex ``u \in V``
02. Exclude ``u`` from ``V`` and ``E``. That is, obtain a new graph ``G_{1} = (V_{1}, E_{1})``, subgraph of ``G``, such that ``u`` is neither in ``V_{1}`` nor in any edge from ``E_{1}``
03. Argue that ``\abs{E} = \abs{E_{1}} + 1 \le (\abs{V_{1}} - 1) + 1 = \abs{V} - 1``

We soon realize that not every vertex of ``V`` can play the role of ``u`` above. Indeed, for the third argument above to work, we need ``\deg(u) = 1``.
In other words, we would like to pick ``u`` such that it is adjent to only one other vertex. Now, we give the argument that such ``u`` exists. More precisely, we show that
```math
\exists\, u \in V \;\text{s.t.}\; \deg(u) \le 1.
```
This can be shown using proof by contradiction: Assume the contrary, i.e. that every vertex in ``V`` has degree greater than or equal to ``2``. Then starting from any vertex ``v`` of ``V``, which we put at the left of the following figure, we will always arrive at the conclusion that there exists a cycle in ``V``.


Once the existence of ``u`` s.t. ``\deg(u) \le 1`` is established, it is easy to prove the rest.

- If ``\deg(u) = 0``, then removing ``u`` makes the aforementioned ``G_{1} = (V_{1}, E_{1})`` an acyclic graph of ``k`` vertices, whence
  ```math
  \abs{E} = \abs{E_{1}} \le (\abs{V} - 1) - 1 \le \abs{V} - 1.
  ```
- Similarly, if ``\deg(u) = 1``, then removing ``u`` makes ``G_{1}`` an acyclic graphs of ``k`` vertices, whence
  ```math
  \abs{E} = \abs{E_{1}} + 1 \le (\abs{V_{1}} - 1) + 1 = \abs{V_{1}} = \abs{V} - 1.
  ```
This concludes the proof.
"""

# ╔═╡ 684a24c0-9ca9-46d6-923f-f35c9129d221


# ╔═╡ 1a24de70-4203-475d-b183-086c6e0e392c


# ╔═╡ 4e5a7a11-397a-4f02-835f-cf56ff8b2ead


# ╔═╡ 01a0665d-794c-4f72-80cc-237c59753f94
